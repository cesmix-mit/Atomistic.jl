...
temp = @time plot_temperature(eq_result, 1)
energy = @time plot_energy(eq_result, 1)
...
display(@time plot_temperature!(temp, prod_result, 1))
display(@time plot_energy!(energy, prod_result, 1))

rdf = @time plot_rdf(prod_result, potential.σ, Int(0.50 * prod_steps))
display(rdf)
