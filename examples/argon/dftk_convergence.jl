...
initial_system = attach_psp(generate_atoms_in_cubic_cell(N, element, box_size, reference_temp); functional="lda")
...
energies = @time map(cutoffs) do cutoff
    @info "Ecut: $(cutoff)"
    dftk_potential = DFTKPotential(cutoff, [1, 1, 1]; functionals=[:lda_x, :lda_c_pw], scf_kwargs=Dict(:damping => 0.7))
    InteratomicPotentials.potential_energy(system, dftk_potential)
end

display(plot(title="DFTK Convergence Analysis", xlab="Ecut", ylab="Total Energy", legend=false, cutoffs, energies * u"hartree"))
