...
@testset "implementations/nbodysimulator/nbodysimulator_base.jl" begin
    ...
    @test AtomsBase.velocity(atom3) ≈ (@SVector [5, 6, 7])u"bohr * hartree / ħ_au"
    ...
    @test AtomsBase.velocity(atom4) ≈ (@SVector [1.0, 0.5, 0.0])u"bohr/s"
    @test atomic_mass(atom4) ≈ atomic_mass(atom1)
    @test AtomsBase.atomic_symbol(atom4) == AtomsBase.atomic_symbol(atom1)
